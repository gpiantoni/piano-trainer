import type { ExpectedEvent, NoteEvent } from '../types.ts';

// Wait mode: no clock. The score waits on the current chord (all events sharing
// an onset) until every pitch in it has been pressed, then advances.

export type NoteState = 'current' | 'hit';
...
export class WaitMode {
  ...
  constructor(timeline: ExpectedEvent[]) {
    this.chords = groupChords(timeline);
    this.enter(0);
  }
  ...
  noteStates(): Map<string, NoteState> {
    ...
    for (const ev of this.current?.events ?? []) {
      mark(ev, this.pending.has(ev.pitch) ? 'current' : 'hit');
    }
    return states;
  }
